...
/** 16-button gamepad report (Android TV KEYCODE_BUTTON_1..16). */
typedef struct {
  uint8_t buttons[2];
} GamepadReport;

/** Arbitrary 16-bit HID Consumer-Page usage value (e.g. 0x008D GUIDE,
 *  0x009C CHANNEL_UP, 0x019C PROFILE_SWITCH). Sending 0x0000 releases. */
typedef struct {
  uint16_t usage;
} ConsumerUsageReport;
...
private:
  ...
  NimBLECharacteristic* inputConsumerUsage;
  NimBLEAdvertising*    advertising;
  KeyReport             _keyReport;
  MediaKeyReport        _mediaKeyReport;
  GamepadReport         _gamepadReport;
  ConsumerUsageReport   _consumerReport;
...
public:
  ...
  void sendReport(GamepadReport* keys);
  void sendReport(ConsumerUsageReport* report);
  /** Press gamepad button 1–16 (maps to KEYCODE_BUTTON_N on Android TV). */
  void writeGamepadButton(uint8_t button1to16);
  /**
   * Press-and-release a raw HID Consumer Page (page 0x0C) usage code, e.g.
   *   0x008D GUIDE, 0x009C CHANNEL_UP, 0x019C PROFILE_SWITCH,
   *   0x0041 Menu Pick (→ Linux KEY_SELECT → Android DPAD_CENTER), etc.
   * The full list of usages a particular TV will accept is determined by its
   * Vendor_VVVV_Product_PPPP.kl file.
   */
  void writeConsumerUsage(uint16_t usage);
