...
static const uint8_t _hidReportDescriptor[] = {
  ...
  END_COLLECTION(0),                 // END_COLLECTION
  // ------------------------------------------------- 16-button report
  // Declared as a Vendor-Defined application (not "Game Pad") so the Linux HID
  // layer routes button N → BTN_MISC + (N-1) = BTN_0..BTN_15 (256..271).
  // Android's Vendor_0484_Product_5738.kl maps those to BUTTON_1..BUTTON_16,
  // which Google TV's launcher binds to Netflix / YouTube / Prime / Disney+
  // (`BUTTON_3..BUTTON_7`).  With "Game Pad" the kernel would instead emit
  // BTN_A / BTN_B / BTN_X / BTN_Y (304+), which the launcher ignores.
  USAGE_PAGE(2),      0x00, 0xFF,    // USAGE_PAGE (Vendor-defined 0xFF00)
  USAGE(1),           0x01,          // USAGE (Vendor 1 — opaque "buttons" device)
  COLLECTION(1),      0x01,          // COLLECTION (Application)
  ...
  HIDINPUT(1),        0x02,
  END_COLLECTION(0),
  // ------------------------------------------------- Generic 16-bit Consumer Page usage
  // Lets us emit any HID consumer usage (0x000..0x3FF) so the kernel can produce
  // KEY_VOLUMEUP / KEY_SELECT / KEY_GUIDE / KEY_CHANNELUP / KEY_PROFILE_SWITCH …
  // and the TV's Vendor_*.kl file can map them to KEYCODE_* properly.
  USAGE_PAGE(1),      0x0C,          // USAGE_PAGE (Consumer)
  USAGE(1),           0x01,          // USAGE (Consumer Control)
  COLLECTION(1),      0x01,          // COLLECTION (Application)
  REPORT_ID(1),       CONSUMER_USAGE_ID,
  LOGICAL_MINIMUM(1), 0x00,
  LOGICAL_MAXIMUM(2), 0xFF, 0x03,    // 0x03FF
  USAGE_MINIMUM(1),   0x00,
  USAGE_MAXIMUM(2),   0xFF, 0x03,    // 0x03FF
  REPORT_SIZE(1),     0x10,          // 16 bits per value
  REPORT_COUNT(1),    0x01,          // one usage at a time
...
void BleKeyboard::begin(void)
{
  ...
  inputConsumerUsage = hid->inputReport(CONSUMER_USAGE_ID);
  #else
  ...
  inputConsumerUsage = hid->getInputReport(CONSUMER_USAGE_ID);
  #endif
...
void BleKeyboard::sendReport(GamepadReport* keys)
{
  if (this->isConnected())
  {
    this->inputGamepad->setValue((uint8_t*)keys, sizeof(GamepadReport));
    this->inputGamepad->notify();
    this->delay_ms(_delay_ms);
  }
}

void BleKeyboard::writeGamepadButton(uint8_t button1to16)
{
  if (button1to16 < 1 || button1to16 > 16) return;
  const uint8_t bit = button1to16 - 1;
  _gamepadReport.buttons[0] = 0;
  _gamepadReport.buttons[1] = 0;
  if (bit < 8) {
    _gamepadReport.buttons[0] = (uint8_t)(1 << bit);
  } else {
    _gamepadReport.buttons[1] = (uint8_t)(1 << (bit - 8));
  }
  sendReport(&_gamepadReport);
  _gamepadReport.buttons[0] = 0;
  _gamepadReport.buttons[1] = 0;
  sendReport(&_gamepadReport);
}

void BleKeyboard::sendReport(ConsumerUsageReport* report)
{
  if (this->isConnected())
  {
    /* Little-endian 16-bit value; NimBLE prepends the report ID byte itself. */
    uint8_t buf[2] = { (uint8_t)(report->usage & 0xFF),
                       (uint8_t)((report->usage >> 8) & 0xFF) };
    this->inputConsumerUsage->setValue(buf, sizeof(buf));
    this->inputConsumerUsage->notify();
    this->delay_ms(_delay_ms);
  }
}

void BleKeyboard::writeConsumerUsage(uint16_t usage)
{
  _consumerReport.usage = usage;
  sendReport(&_consumerReport);
  _consumerReport.usage = 0x0000;
  sendReport(&_consumerReport);
}
