...
function defaultDeviceApi() {
  ...
  const onDevice =
    ...
    /^192\.168\./.test(host) ||
    /^10\./.test(host);
  if (onDevice && (location.port === '' || location.port === '80')) {
    return `${location.protocol}//${location.host}`;
  }
  return localStorage.getItem(API_BASE_KEY) || 'http://omote.local';
}

let API = defaultDeviceApi();
...
let haBrowserLastError = '';

function getHaCredentials() {
  const prefs = loadEditorPrefs();
  const url = ($('ha-url')?.value || prefs.ha_url || '').trim().replace(/\/+$/, '');
  const token = ($('ha-token')?.value || prefs.ha_token || '').trim();
...
/** Home Assistant REST from the browser — keeps heavy traffic off the ESP32. */
async function haBrowserFetch(path, opts = {}) {
  const creds = getHaCredentials();
  if (!creds) throw new Error('HA URL and token required in Setup');
  const timeoutMs = opts.timeoutMs || 8000;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const r = await fetch(`${creds.url}${path}`, {
...
function parseHaTemplateEntities(text, search) {
  ...
  for (const line of String(text).split('\n')) {
    ...
    if (!entity_id.includes('.')) continue;
    const state = parts[1].trim();
    const friendly_name = (parts[2] || '').trim();
...
function haBrowserErrorHint(err) {
  ...
  if (m === 'Failed to fetch' || err?.name === 'TypeError') {
    return (
      'Browser cannot reach Home Assistant (CORS or wrong URL). ' +
      'Use http://YOUR_HA_IP:8123 and add this editor URL to homeassistant http.cors_allowed_origins.'
    );
  }
...
async function haBrowserListEntitiesFromStates(domain, search) {
  ...
  try {
    ...
    throw new Error('HA returned invalid JSON from /api/states');
  }
  ...
  for (const s of states) {
    ...
    entities.push({
      ...
      state: s.state || '',
      friendly_name,
      icon: s.attributes?.icon && s.attributes.icon !== 'null' ? s.attributes.icon : '',
      domain: dom,
    });
...
async function haBrowserListEntitiesTemplate(domain, search) {
  const dom = domain || 'light';
  const tpl =
    `{% for s in states %}{% if s.entity_id.split('.')[0] == '${dom}' %}` +
    `{{ s.entity_id }}|{{ s.state }}|{{ s.attributes.friendly_name | default('') }}|` +
    `{{ s.attributes.icon | default('') }}\n{% endif %}{% endfor %}`;
  const text = await haBrowserFetch('/api/template', {
    method: 'POST',
    body: JSON.stringify({ template: tpl }),
  });
  return parseHaTemplateEntities(text, search);
}

async function haBrowserListEntities(domain, search) {
  try {
    return await haBrowserListEntitiesFromStates(domain, search);
  } catch (e) {
    console.warn('HA /api/states failed, trying template API:', e.message);
    return await haBrowserListEntitiesTemplate(domain, search);
  }
}
...
async function haBrowserEntityState(entityId) {
  ...
  return {
    entity_id: entityId,
    state,
    on: isHaStateOn(state),
    friendly_name: data.attributes?.friendly_name || '',
    icon: data.attributes?.icon || '',
  };
}

async function haBrowserPing() {
  await haBrowserFetch('/api/');
  haBrowserLastError = '';
  return true;
}

async function loadHaEntitiesFromBrowser(domain, search) {
...
async function refreshHaToggleStates() {
  ...
  const creds = getHaCredentials();
  for (const b of haBtns.slice(0, 6)) {
    ...
    try {
      ...
      if (creds) {
        try {
          data = await haBrowserEntityState(eid);
        } catch (_) {
          if (deviceConnected) {
            data = await api('/api/ha/entity?entity_id=' + encodeURIComponent(eid));
          } else {
...
function stopHaStatePoll() {
  if (haStatePollTimer) {
    clearInterval(haStatePollTimer);
    haStatePollTimer = null;
  }
}
...
async function api(path, opts = {}) {
  ...
  if (!base) throw new Error('Set Device URL in Setup (e.g. http://omote.local)');
  const { timeoutMs = 12000, headers: extraHeaders, ...fetchOpts } = opts;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    if (!r.ok) {
      const err = data?.error || (typeof data === 'string' ? data : r.statusText);
      if (r.status === 503) {
        throw new Error(
          `${err} Close this editor tab while using the physical remote, or free device memory.`
        );
      }
      throw new Error(err);
    }
    ...
    if (e.name === 'AbortError') throw new Error(`Device request timed out (${timeoutMs / 1000}s)`);
    const m = String(e.message || e);
    if (m === 'Failed to fetch' || e.name === 'TypeError') {
      throw new Error(
        `Cannot reach Omote at ${base}. Check Device URL and WiFi. ` +
          `Right after Deploy the device reboots — that can look like this error; wait 30s and refresh.`
      );
    }
    throw e;
  } finally {
...
function isLikelyDeployDisconnect(err) {
  const m = String(err?.message || err || '');
  return m === 'Failed to fetch' || err?.name === 'TypeError' || /NetworkError|reboots/i.test(m);
}
...
function parseBackupText(text) {
  let data = JSON.parse(text.trim().replace(/^\uFEFF/, ''));
  let repaired = false;
...
function normalizeKeyChar(key) {
  if (key == null) return '';
  const s = String(key);
  if (s.length === 1) return s;
  const lower = s.toLowerCase();
  if (lower === 'power' || lower === 'tv_power' || lower === 'key_power') return POWER_KEY;
  return s[0];
}
...
function normalizeConfigShape(cfg) {
  ...
  for (const kb of out.keymap) {
    if (kb?.key != null) kb.key = normalizeKeyChar(kb.key);
  }
  for (const page of out.pages) {
    if (!page?.keys || typeof page.keys !== 'object') continue;
    const fixed = {};
    for (const [k, act] of Object.entries(page.keys)) {
      fixed[normalizeKeyChar(k)] = act;
    }
    page.keys = fixed;
  }
  return out;
...
async function loadHaEntities(opts) {
  ...
  try {
    ...
    const creds = getHaCredentials();
    if (!creds) {
      status.textContent = 'Enter HA URL + token in Setup tab (entities load from HA in your browser).';
      status.classList.add('error');
      return;
    }
    try {
      ({ data, via } = await loadHaEntitiesFromBrowser(domain, q));
    } catch (e) {
      haBrowserLastError = haBrowserErrorHint(e);
      console.warn('HA browser load failed:', e);
      if (!deviceConnected) {
        status.textContent = haBrowserLastError;
        status.classList.add('error');
        return;
      }
      const dom = encodeURIComponent(domain);
      const qs = encodeURIComponent(q);
      data = await api(`/api/ha/entities?domain=${dom}&search=${qs}`);
      via = 'device';
    }
    status.textContent = `${(data.entities || []).length} entities (via ${via === 'browser' ? 'Home Assistant' : 'Omote'})`;
    ...
    if (!status.textContent.includes('entities')) {
...
function startLearnMode() {
  ...
  learnPoll = setInterval(async () => {
    if (document.hidden) return;
    try {
      const ev = await api('/api/key/last', { timeoutMs: 3000 });
...
$('btn-export-backup').onclick = async () => {
  try {
    $('setup-msg').textContent = 'Exporting from device (may take a few seconds)…';
    let data = await api('/api/backup/export', { timeoutMs: 45000 });
    if (typeof data === 'string') {
      try {
        data = JSON.parse(data);
      } catch {
...
$('backup-file').onchange = async (ev) => {
  ...
  try {
    ...
    msg.textContent = `Loaded in editor — ${summary}${repairNote}`;
    msg.classList.remove('error');
    if (editorMsg) {
      editorMsg.textContent = msg.textContent;
      editorMsg.classList.remove('error');
    }
    loadEntities().catch(() => {});

    if (!deviceConnected) {
      msg.textContent += '. Connect to Omote and Deploy when ready.';
      ev.target.value = '';
      return;
    }

    const payload = buildBackupPayload(data);
    msg.textContent = `Loaded in editor. Uploading to Omote…`;
    try {
      await api('/api/backup/import', { method: 'POST', body: payload, timeoutMs: 90000 });
      try {
        msg.textContent = 'On device — syncing editor…';
        await reloadFromDeviceAfterImport();
        msg.textContent = `Backup restored on device — ${summary}${repairNote}`;
      } catch (reloadErr) {
        msg.textContent =
          `Uploaded to device — ${summary}${repairNote}. Editor sync: ${reloadErr.message}. Try Deploy.`;
      }
    } catch (uploadErr) {
      msg.textContent =
        `Loaded in editor — ${summary}. Device upload failed: ${uploadErr.message}. Use Deploy on Layout.`;
      msg.classList.add('error');
    }
    if (editorMsg) editorMsg.textContent = msg.textContent;
    ev.target.value = '';
  } catch (e) {
...
$('btn-test-ha').onclick = async () => {
  ...
  try {
    const creds = getHaCredentials();
    if (!creds) {
      throw new Error('Enter HA URL and long-lived token above first.');
    }
    msg.textContent = 'Testing HA from this browser…';
    await haBrowserPing();
    const sample = await haBrowserListEntities('light', '');
    msg.textContent =
      `HA OK (${sample.entities.length} lights found). Saving URL/token to Omote…`;
    msg.classList.remove('error');
    if (deviceConnected) {
      try {
        await api('/api/settings', {
          ...
          body: JSON.stringify({
            ha_url: creds.url,
            ha_token: creds.token,
          }),
        });
        saveEditorPrefs({ ha_url: creds.url, ha_token: creds.token });
        msg.textContent =
          `HA OK — ${sample.entities.length} entities visible. Credentials saved on Omote.`;
      } catch (e) {
        msg.textContent =
          `HA OK in browser, but Omote save failed: ${e.message}. Fix Device URL / WiFi.`;
        msg.classList.add('error');
      }
    }
    await loadEntities();
  } catch (e) {
    msg.textContent = haBrowserErrorHint(e);
    msg.classList.add('error');
  }
};
...
$('btn-deploy').onclick = async () => {
  ...
  try {
    ...
    let deployOk = false;
    try {
      const res = await api('/api/config', { method: 'POST', body: payload, timeoutMs: 90000 });
      deployOk = res?.ok === true || res?.restart === true;
    } catch (e) {
      if (isLikelyDeployDisconnect(e)) deployOk = true;
      else throw e;
    }
    if (!deployOk) throw new Error('Deploy was not acknowledged by the device.');
    $('editor-msg').textContent = 'Deploy accepted — device rebooting…';
    deviceConnected = false;
    for (let i = 0; i < 40; i++) {
      await sleep(1000);
...
async function init() {
  const prefs = loadEditorPrefs();
  const defApi = defaultDeviceApi();
  API = prefs.device_url || localStorage.getItem(API_BASE_KEY) || defApi;
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    stopHaStatePoll();
    if (learnActive) stopLearnMode('Paused — tab in background');
    return;
  }
  if (!deviceConnected) {
    connectToDevice({ retries: 8, silent: true });
    return;
  }
  if ($('tab-layout')?.classList.contains('active')) startHaStatePoll();
});
